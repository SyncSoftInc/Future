...
namespace SyncSoft.Future.Logistics.Domain.Inventory.HoldOrderInventories
{
    ...
    public class MasterDBActivity : TccActivity
    {
        ...
        protected override async Task RunAsync(CancellationToken? cancellationToken)
        {
            ...
            // 锁库存，得到分配后计算得到的可用库存
            var availableInventories = await inventoryMasterDAL.HoldOrderInventoriesAsync(cmd).ConfigureAwait(false);
        ...
        protected override async Task RollbackAsync()
        {
            ...
            var inventoryMasterDAL = await LogisticsMasterDALFactory.CreateInventoryDALAsync(cmd.Merchant_ID).ConfigureAwait(false);
            // 回滚，解锁库存
            await inventoryMasterDAL.UnholdOrderInventoriesAsync(rollabckCmd).ConfigureAwait(false);
        }
    }
}
